...
class NukiApi:
    ...
    async def _auth_headers(self) -> dict[str, str]:
        """Build auth headers from HA OAuth2Session.

        Fix: async_ensure_token_valid() can return None -> never call .get() on it.
        Raise ConfigEntryAuthFailed so HA can reauth the entry.
        """
    ...
    async def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        ...
        Fix: if Nuki returns 401/403, raise ConfigEntryAuthFailed (reauth).
        """
        ...
        async with session.request(method, url, headers=headers, **kwargs) as resp:
            if resp.status in (401, 403):
                # Force HA to treat as auth issue rather than silent update failure loops
            ...
            # Nuki API responses are JSON for the endpoints we use
