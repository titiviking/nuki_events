...
# Stable logger name so this always works:
# logger:
#   logs:
#     custom_components.nuki_events.coordinator: debug
...
class NukiDataCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        """Fetch data from Nuki.

        HA will often only show "success: False" for UpdateFailed.
        We log the underlying exception with traceback so the real cause is visible.
        """
        try:
            ...
            if locks is None:
                _LOGGER.error("api.list_smartlocks() returned None")
                locks = []
            else:
                try:
                    _LOGGER.debug("Fetched %s locks", len(locks))
                except Exception:
                    _LOGGER.debug("Fetched locks (non-sized type=%s)", type(locks).__name__)

            prior = self.data or {}
            auth_map: dict[str, str] = prior.get("auth_map", {})

            try:
                auths = await self.api.get_all_auths()
                if auths is None:
                    _LOGGER.debug("api.get_all_auths() returned None (skipping)")
                    auths = []
                for a in auths or []:
                ...
                _LOGGER.debug("Auth map size: %s", len(auth_map))
            except Exception:
                # Don’t fail the whole update because auths failed — keep prior map.
                _LOGGER.exception("Auth list refresh failed (continuing with prior auth_map)")
            ...
            # This is the missing piece: log the real traceback.
    ...
    async def async_handle_webhook(self, entry_id: str, payload: dict[str, Any]) -> None:
        ...
        await self.async_request_refresh()    5: "lock_n_go_unlatch",
    208: "door_sensor_jammed",
    209: "door_sensor_error",
    224: "keypad_battery_critical",
    225: "keypad_battery_low",
